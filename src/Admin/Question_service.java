...
public class Question_service {
    ...
    @Path("add")
    public String add(@FormParam("param1") String sub, @FormParam("param2") int m, @FormParam("param3") int r, @FormParam("param4") int c)
    {
        try {
    ...
    @Path("viewAll")
    @Produces(MediaType.TEXT_PLAIN)
    public String viewAll(@FormParam("param1") String sid)
    {
...
//        java.util.List<Student> tlist=session1.createQuery("select s.id,s.name,s.roll,s.CSClass,s.division from Student s").list();
        Squestion m= (Squestion) session1.createQuery("from Squestion s where s.subject.id=:sid and s.qtype=:qid").setParameter("sid",sid).setParameter("qid",1).uniqueResult();
        Squestion r= (Squestion) session1.createQuery("from Squestion s where s.subject.id=:sid and s.qtype=:qid").setParameter("sid",sid).setParameter("qid",2).uniqueResult();
        Squestion c= (Squestion) session1.createQuery("from Squestion s where s.subject.id=:sid and s.qtype=:qid").setParameter("sid",sid).setParameter("qid",3).uniqueResult();
        t1.commit();
        session1.close();
...
//        return String.valueOf(squestion.getTotal());
    ...
    @Path("getMSq")
    @Produces(MediaType.TEXT_PLAIN)
    public String getMSq(@FormParam("param1") String sid)
    {
        Session session1= DB.Global.getSession();
        Transaction t1=session1.beginTransaction();
        Squestion m= (Squestion) session1.createQuery("from Squestion s where s.subject.id=:sid and s.qtype=:qid").setParameter("sid",sid).setParameter("qid",1).uniqueResult();
    ...
    @Path("getRSq")
    @Produces(MediaType.TEXT_PLAIN)
    public String getRSq(@FormParam("param1") String sid)
    {
        Session session1= DB.Global.getSession();
        Transaction t1=session1.beginTransaction();
        Squestion r= (Squestion) session1.createQuery("from Squestion s where s.subject.id=:sid and s.qtype=:qid").setParameter("sid",sid).setParameter("qid",2).uniqueResult();
    ...
    @Path("getCSq")
    @Produces(MediaType.TEXT_PLAIN)
    public String getCSq(@FormParam("param1") String sid)
    {
        Session session1= DB.Global.getSession();
        Transaction t1=session1.beginTransaction();
        Squestion c= (Squestion) session1.createQuery("from Squestion s where s.subject.id=:sid and s.qtype=:qid").setParameter("sid",sid).setParameter("qid",3).uniqueResult();
